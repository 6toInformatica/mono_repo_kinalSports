    ...
    ILogger<AuthService> logger) : IAuthService
{
        ...
        IFileData profilePicture)
    {
        ...
        var (isValid, errorMessage) = FileValidator.ValidateImage(profilePicture);
        if (!isValid)
        {
            throw new BusinessException(ErrorCodes.INVALID_FILE_FORMAT, errorMessage!);
        }

        var fileName = FileValidator.GenerateSecureFileName(profilePicture.FileName);
        string profilePicturePath;

        try
        {
            profilePicturePath = await _cloudinaryService.UploadImageAsync(
                profilePicture,
                fileName);
        }
        catch (Exception)
        ...
        var previousPicture = user.UserProfile?.ProfilePicture ?? string.Empty;
        if (
            !string.IsNullOrWhiteSpace(previousPicture)
            && !previousPicture.Contains("avatarDefault", StringComparison.OrdinalIgnoreCase)
            && !previousPicture.StartsWith("http", StringComparison.OrdinalIgnoreCase))
        {
            await _cloudinaryService.DeleteImageAsync(previousPicture);
        }
